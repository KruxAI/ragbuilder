...
def codegen(**kwargs):
    ...
    print('return_code',kwargs['return_code'])
    llm = getLLM(**kwargs)
    docs = ragbuilder_loader(**kwargs)
    code_text = "\n\n" +llm + "\n\n" +docs
    with open("code.py", 'w') as f:
        f.write(code_text)
codegen(retrieval_model="gpt-3.5-turbo",input_path="https://en.wikipedia.org/wiki/Python_(programming_language)",source_ids=["wiki"])
