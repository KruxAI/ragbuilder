...
#         # Prompt setup
#         rewrite_prompt = '''You are an AI assistant tasked with reformulating user queries to improve retrieval in a RAG system. 
...
# print(rag_pipeline().invoke("what is RAG"))
...
6. **Few-shot**  
   You are an expert assistant. Use the provided context to answer the question in a detailed and structured manner.  
   Ensure your response aligns strictly with the context while elaborating on key concepts sequentially.  
   Begin by introducing the overarching idea, then explain the process step-by-step, and conclude with its impact.  
   Avoid adding any information not present in the context.  
   If the context does not contain sufficient information, state, "The context does not provide enough information to answer."  
   Example:  
   Q: {question}  
   A: ...
...
Here are the Question, Answer and Ground Truth. Generate a RAG prompt using the above formats in such a way that the answer is as close to the ground truth as possible. Respond with only the prompt format
QUESTION: {question}
