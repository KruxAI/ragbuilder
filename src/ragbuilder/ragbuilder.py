...
@app.post("/create_chat_session/{eval_id}")
async def create_chat_session(eval_id: int, db: sqlite3.Connection = Depends(get_db)):
    try:
        rag_builder = rag_manager.get_rag(eval_id, db)
        session_id = f"{eval_id}_{int(time.time())}"
